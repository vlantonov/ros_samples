...
#include <actionlib_callback_based/FibonacciAction.h>
#include <ros/ros.h>

class FibonacciAction {
 protected:
  ros::NodeHandle nh_;
  actionlib::SimpleActionServer<actionlib_callback_based::FibonacciAction>
      as_;  // NodeHandle instance must be created before this line. Otherwise
            // strange error occurs.
  std::string action_name_;
  // create messages that are used to published feedback/result
  actionlib_callback_based::FibonacciFeedback feedback_;
  actionlib_callback_based::FibonacciResult result_;
 ...
 public:
  ...
  void executeCB(const actionlib_callback_based::FibonacciGoalConstPtr& goal) {
    // helper variables
    ros::Rate r(1);
    bool success = true;
